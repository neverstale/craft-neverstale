...
class Settings extends Model
{
    ...
    /**
     * Get bulk ingest max items with config and ENV override support
     */
    ...
    /**
     * Get bulk ingest batch size with config and ENV override support
     */
    public function getBulkIngestBatchSize(): int
    {
        $configValue = Plugin::getInstance()->config->get('bulkIngestBatchSize');
        if ($configValue !== null) {
            return (int) $configValue;
        }
    ...
    /**
     * Get bulk ingest max concurrency with config and ENV override support
     */
    public function getBulkIngestMaxConcurrency(): int
    {
        $configValue = Plugin::getInstance()->config->get('bulkIngestMaxConcurrency');
        if ($configValue !== null) {
            return (int) $configValue;
        }
    ...
    /**
     * Get bulk ingest retry attempts with config and ENV override support
     */
    public function getBulkIngestRetryAttempts(): int
    {
        $configValue = Plugin::getInstance()->config->get('bulkIngestRetryAttempts');
        if ($configValue !== null) {
            return (int) $configValue;
        }
    ...
    /**
     * Get bulk ingest timeout minutes with config and ENV override support
     */
    public function getBulkIngestTimeoutMinutes(): int
    {
        $configValue = Plugin::getInstance()->config->get('bulkIngestTimeoutMinutes');
        if ($configValue !== null) {
            return (int) $configValue;
        }
    ...
    /**
     * Get bulk ingest enabled with config and ENV override support
     */
    public function getBulkIngestEnabled(): bool
    {
        $configValue = Plugin::getInstance()->config->get('bulkIngestEnabled');
        if ($configValue !== null) {
    ...
    /**
     * Get debug logging with config and ENV override support
     */
    public function getDebugLogging(): bool
    {
        $configValue = Plugin::getInstance()->config->get('debugLogging');
        if ($configValue !== null) {
            return (bool) $configValue;
        }
    ...
    /**
     * Get webhook domain with config and ENV override support
     */
